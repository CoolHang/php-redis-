...
class rRedis
{
  ...
  public function connect($config=array(), $isMaster=true){
    // default port
    if(!isset($config['port'])) $config['port'] = $GLOBALS['redis']['port'];
    if(!isset($config['host'])) $config['host'] = $GLOBALS['redis']['host'];
    // 设置 Master 连接
    if($isMaster){
   ...
   * @return int　操作后的值
   */
  public function incrfloat($key,$value=1){
   ...
   *
   * 
   * @param int $pattern 格式
   * @return array　符合要求的key
   */
  ...
  public function lindex($name, $index){
    return $this->getRedis()->lindex($name,$index);
  }
  ...
  public function lset($key,$index,$value){
    if(is_array($value)){
      return $this->getRedis()->lset($key,$index,serialize($value));  
    }
  ...
  /**
   *  set hash opeation 一维数组
   */
  public function hsetarr($name,$data){
    foreach($data as $key=>$val)
    {
        $this->hset($name, $key, $val);
    }
  }
   ...
   * @return int　操作后的值
   */
  public function hincr($key, $field, $default=1){
    if(is_float($default)){
      return $this->getRedis()->hincrbyfloat($key, $field, $default);
    }else{
      return $this->getRedis()->hincrby($key, $field, $default);
    }
  }
